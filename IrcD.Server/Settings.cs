...
namespace IrcD.Server
{
    public class Settings
    {
        ...
        public IrcMode GetIrcMode()
        {
            IrcMode defaultmode = IrcMode.Modern;

            var tmode = configFile.Descendants("irc_mode").Select(m => m.Value).FirstOrDefault();
            if(tmode == default(string))
                return defaultmode;

            IrcMode result;
            if(Enum.TryParse<IrcMode>(tmode, true, out result))
            {
                return result;
            }
        ...
        public void setDaemon(IrcDaemon ircDaemon)
        {
